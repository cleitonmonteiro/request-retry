...
- **Mapping a wire enum through the layers** (`domain/model/Action.kt`, `data/remote/ActionDto.kt`, `data/mapper/ActionMapper.kt`): the send-item response carries an SDUI-style "next action" (`ActionType.DEEPLINK` / `EXTERNAL_LINK` / `CLOSE`, plus an optional `target`). The pattern, in order: (1) a **DTO-side enum** (`ActionTypeDto`) whose constants match the wire's actual spelling via per-entry `@SerialName` — exactly like `@SerialName` on a DTO property, but on an enum entry instead; (2) a **plain domain enum** (`ActionType`) with no serialization annotations, so the domain layer stays wire-agnostic; (3) an explicit, **exhaustive `when` with no `else`** in the mapper translating DTO constant → domain constant, so adding a new wire case is a compile error everywhere it isn't handled, not a silent runtime gap. `PickerScreen`'s `SendConfirmation` then branches on `Action.type` to decide what the UI actually does (open a deeplink/external link via `Intent.ACTION_VIEW`, or call `onLeave()` for `CLOSE`) — the server drives the client's next step instead of the client hardcoding it.
