...
- `data/` implements the domain contracts against a mocked backend:
  ...
  - `repository/*RepositoryImpl.kt` — inject a data source, wrap the still-`suspend` fetch in `flow { emit(...) }`, map DTO → domain. Exceptions are never swallowed here; `RetryController`'s `.catch` is what catches them, at **collection** time (not when the use case is called). **Do not add `.flowOn(Dispatchers.IO)` here** — `FakeNetwork` only calls `delay()`, and `flowOn` would move that off the test dispatcher, breaking every test's virtual time.
- `di/RepositoryModule.kt` — `@Binds` for the two repository interfaces. `di/RetryModule.kt` — `@Provides` the app's single `RetryPolicy` (`ExponentialBackoffPolicy`, `@Singleton`; needed because `RetryPolicy` is a `fun interface` Dagger can't construct on its own). Everything else (use cases, data sources, `FakeNetwork`, `RetryControllerFactory`, the ViewModels) is plain constructor injection, discovered automatically.
...
- `PickerViewModelTest` wires its real repository chain **twice** (once per use case) rather than sharing one instance — that mirrors what Hilt actually builds (see `feature/picker/` above) and is what makes its independence test honest.
